...
const char* ssid = "ATT5vJjjbi";
const char* password = "t8+x9qqjpi8c";
...
void sendImageToDjangoServer() {
  ...
  String serverName = "http://192.168.1.216:8000/upload/";
  ...
  bodyStart += "Content-Disposition: form-data; name=\"image\"; filename=\"esp32.jpg\"\r\n";
  bodyStart += "Content-Type: image/jpeg\r\n\r\n";

  String bodyEnd = "\r\n--" + boundary + "--\r\n";

  // Allocate buffer for whole body
  ...
  if (httpCode > 0) {
    ...
    Serial.printf("Failed to send image. Error: %d\n", httpCode);
  }
...
void loop() {
    // Only print the stream info once at boot (optional)
    ...
    // Every 7 seconds, send an image
    if (millis() - lastSendTime > sendInterval) {
        lastSendTime = millis();
        Serial.println("Capturing and sending image...");
        sendImageToDjangoServer();
    }
